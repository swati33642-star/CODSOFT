...
def preprocess(df):
    ...
    # Save scalers + the exact column order the model expects, so the
    # Streamlit app (or any other consumer) can transform new raw
    # transactions the same way before predicting.
    joblib.dump(
        {
        ...
        f"{OUTPUT_DIR}/preprocessing.pkl",
    )
